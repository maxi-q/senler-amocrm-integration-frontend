...
export const OAuth2token = async ({client_secret, redirect_uri, code, group_id}: {client_id: string, client_secret: string, redirect_uri: string, code: string, group_id: string}) => {
  try {
     const response = await axios.get<{success: boolean, access_token: string}>(
      `https://senler.ru/ajax/cabinet/OAuth2token`,
      {
        params: {
          client_id: '670b92d90d647d1fc4350042',
